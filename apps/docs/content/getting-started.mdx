...
binaries:
  ...
  kubectl:
    version: "v1.28.0"
    enforced: true          # strict pin — never auto-upgrade
...
Use the `enforced` field to strictly pin a version. Unlike `version` (which can be auto-upgraded by `b update`), an enforced version is locked and never automatically changed.
...
For private repositories or to increase API rate limits, set provider-specific tokens as environment variables:
...
- [Authentication](/authentication) - Configure tokens for private repos