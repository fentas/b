...
- **Simple dot-paths** like `binaries`, `.binaries`, `database.host`, or even plain keys with characters that the legacy validator accepts (`foo/bar`, `my+key`, `a@b`) are extracted via the YAML Node API. Whole top-level keys keep their comments, ordering, and block/flow style. (`#` is fine for the classifier itself, but in `b.yaml` you'd need to quote a selector starting with `#` like `"#tag"` so YAML doesn't treat it as a comment.)
...
>
> Plain keys with `/`, `+`, `@`, `#`, etc. work without quoting because
> the classifier passes them through to the Node API path.
