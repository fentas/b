...
**Known limitation:** the underlying 3-way merge is line-based
(`git merge-file`), so concurrent *adjacent* additions inside the scoped
region — e.g. local adds `argsh`, upstream adds `tilt` — can produce a
spurious conflict marker even though the changes are semantically
independent. The data-loss bug (losing out-of-scope content) is fixed;
the cosmetic spurious-conflict issue is tracked as a follow-up.
