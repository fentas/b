...
`--dry-run` always prints the plan and never writes. Combine with `--plan-json` to emit a machine-readable plan for PR comment bots and CI summary jobs:
...
Every `b update` produces a **plan** before applying — a per-file table of additions, updates, keeps, overwrites, merges, and conflicts. The `safety` setting controls what `b` does with that plan:

| Mode       | Interactive (TTY)              | Non-interactive (CI)              | Use when                                |
|------------|--------------------------------|-----------------------------------|-----------------------------------------|
| `strict`   | Refuses any destructive plan   | Refuses any destructive plan      | Locked-down repos, untrusted upstreams  |
| `prompt`*  | Shows plan, asks `[y/N]`       | Refuses destructive (= strict)    | Default — safe everywhere               |
| `auto`     | Applies without prompting      | Applies without prompting         | Trusted upstream, fully automated runs  |
