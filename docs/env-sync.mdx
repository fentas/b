...
> simple dot-path classifier will route it to JMESPath where you'll need
> to use a quoted identifier in the JMESPath expression:
> `'"my key with spaces"'` or `'"weird[name]"'`. Plain keys with `/`,
> `+`, `@`, `#`, etc. work without quoting because the classifier passes
> them through to the Node API path.
