...
- **Simple dot-paths** like `binaries`, `.binaries`, `database.host`, or even plain keys with characters that the legacy validator accepts (`foo/bar`, `my+key`, `a@b`, `#tag`) are extracted via the YAML Node API. Whole top-level keys keep their comments, ordering, and block/flow style.
...
> to wrap it in JMESPath bracket-quotes: `'"my key with spaces"'` or
> `'"weird[name]"'`. Plain keys with `/`, `+`, `@`, `#`, etc. work
> without quoting because the classifier passes them through to the
> Node API path.
