...
- **Complex JMESPath expressions** — anything containing brackets, parens, pipes, multi-select hashes, or function calls — are routed to [JMESPath](https://jmespath.org/) (jmespath-community fork, supports `items` / `from_items`). Comments are NOT preserved for keys produced by the complex side.
