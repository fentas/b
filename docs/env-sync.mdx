...
- **Simple dot-paths** like `binaries`, `.binaries`, or `database.host` are extracted via the YAML Node API. Whole top-level keys keep their comments, ordering, and block/flow style.
- **Complex JMESPath expressions** — anything containing brackets, parens, pipes, multi-select hashes, or function calls — are routed to [JMESPath](https://jmespath.org/) (jmespath-community fork, supports `items` / `from_items`). Comments are NOT preserved for the complex output.
- **Mixed lists** are supported: simple selectors and complex selectors on the same file are evaluated independently and merged at the top level. JMESPath wins on key collisions.
...
Note: in the jmespath-community v1.1.1 Go fork, `items()` returns `[key, value]` **tuples**, so accessing the value's fields uses `[1].field` rather than `value.field`. This differs from the JMESPath spec website's examples but matches what the dependency actually ships.

You can also mix simple and complex on the same file:
