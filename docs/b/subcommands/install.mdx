...
Hooks are POSIX shell commands (run via `sh -c`). Non-zero exit produces a warning
but does not fail the install. Output goes to stderr so it doesn't interfere with
progress bars.
